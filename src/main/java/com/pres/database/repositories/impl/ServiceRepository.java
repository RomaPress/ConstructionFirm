    ...
    public Map<Integer, List<Service>> getOrderedService(List<Integer> order_id) {
        ...
        try (Connection connection = getConnection()) {
            for (Integer i : order_id) {
                PreparedStatement statement = connection.prepareStatement("select sro.order_id, sr.service_id, sr.name_service, un.name_unit, sro.amount, sr.unit_price from db_construction_firm.service sr left join db_construction_firm.unit un on sr.unit_id = un.unit_id   right join db_construction_firm.service_order sro on sr.service_id = sro.service_id where sro.order_id = ?;");
                ...
                while (rs.next()) {
                    Service service = new Service();
                    service.setUnit_price(rs.getFloat("unit_price"));
                    service.setService_id(rs.getInt("service_id"));
                    service.setName_service(rs.getString("name_service"));
